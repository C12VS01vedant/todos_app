...
import 'package:todos/todosOverview/bloc/bloc.dart';
import 'package:todos/todosOverview/view/todos_overview_page.dart';
import 'package:todos/todosOverview/widgets/todos_list_tile.dart';
...
void main() {
  ...
  group('TodosOverviewPage', () {
    ...
    testWidgets('subscribes to todos from repository on initialization', (
      tester,
    ) async {
      await tester.pumpApp(
        const TodosOverviewPage(),
  ...
  group('TodosOverviewView', () {
    ...
    testWidgets('renders error snackbar '
    ...
    group('TodoDeletionConfirmationSnackBar', () {
      setUp(() {
        ...
        ).thenReturn(TodosOverviewState(lastDeletedTodo: mockTodos.first));
        whenListen<TodosOverviewState>(
          todosOverviewBloc,
          Stream.fromIterable([
            const TodosOverviewState(),
      ...
      testWidgets('adds TodosOverviewUndoDeletionRequested '
          'to TodosOverviewBloc '
          'when onUndo is called', (tester) async {
        await tester.pumpApp(buildSubject(), todosRepository: todosRepository);
        await tester.pumpAndSettle();
        ...
        verify(
          () =>
              todosOverviewBloc.add(const TodosOverviewUndoDeletionRequested()),
        ).called(1);
      });
    });
    ...
    group('when todos is empty', () {
      ...
      testWidgets('renders nothing '
          'when status is initial or error', (tester) async {
        await tester.pumpApp(buildSubject(), todosRepository: todosRepository);
      ...
      testWidgets('renders todos empty text '
    ...
    group('when todos is not empty', () {
      ...
      testWidgets('adds TodosOverviewTodoCompletionToggled '
          'to TodosOverviewBloc '
          'when TodoListTile.onToggleCompleted is called', (tester) async {
        await tester.pumpApp(buildSubject(), todosRepository: todosRepository);
      ...
      testWidgets('adds TodosOverviewTodoDeleted '
          'to TodosOverviewBloc '
          'when TodoListTile.onDismissed is called', (tester) async {
        await tester.pumpApp(buildSubject(), todosRepository: todosRepository);
      ...
      testWidgets('navigates to EditTodoPage '
          'when TodoListTile.onTap is called', (tester) async {
        await tester.pumpApp(buildSubject(), todosRepository: todosRepository);
